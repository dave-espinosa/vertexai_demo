...
def train_val_test_df_split(df, tr_s, vl_s, ts_s, v=False)
    ...
    # and new test sets.
    val_df = test_df.sample(frac=vl_stt_s)
    test_df.drop(val_df.index, inplace=True)

    if v
        print(ftNumber of rows in training set {len(train_df)})
        print(ftNumber of rows in validation set {len(val_df)})
        print(ftNumber of rows in test set {len(test_df)})
    
    return train_df, val_df, test_df


def make_dataset(df, is_train=True)
    batch_size = 128  # Loca constant, would be better as general
...
print(User-defined functions loaded and ready for use)
...
path = gsonementor-ml-dataprogram_grouping_data.zip
df = pd.read_csv(path, compression='zip')
df['new_skills'] = df['new_skills'].apply(ast.literal_eval)

print(Data loaded and ready for use (shape {}).format(df.shape))
...
print(fttThere are {len(skills_vocabulary)} different terms in the dictionary of 'new_skills')
...
print(fttThere are {len_jobtitle_vocabulary} different terms in the dictionary of 'job_title')
...
jobtitle_vectorizer_layer = tf.keras.layers.TextVectorization(
    max_tokens=int(1.1len_jobtitle_vocabulary),
    output_sequence_length=int(1.1max_len_jobtitle_str),
...
print(fttThere are {len_progroup_vocabulary} different terms in the dictionary of 'program_grouping')
...
input_new_skills = tf.keras.layers.Input(
    shape=(None,),
    dtype=tf.string, 
    ragged=True, 
    name=skill_input
...
x1 = tf.keras.layers.Embedding(
    skill_vectorizer_layer.vocabulary_size(),
    EMBEDDING_SIZE,
    name=skill_embedding
)(x1)

x1 = tf.keras.layers.GlobalAveragePooling1D(
    name=skill_average
)(x1) 

# 'job_title' input branch(x2)

input_job_titles = tf.keras.layers.Input(
    shape=(1,),
    dtype=tf.string,
    name=jobtitle_input
)
...
x2 = tf.keras.layers.Embedding(
    jobtitle_vectorizer_layer.vocabulary_size(), 
    EMBEDDING_SIZE, 
    name=jobtitle_embedding
)(x2)

x2 = tf.keras.layers.GlobalAveragePooling1D(
    name=jobtitle_average
)(x2)

# 'concatenation' of 'x1' & 'x2'
concat = tf.keras.layers.concatenate(
    [x1, x2], axis=-1, name=concatenate
)
...
outputs = tf.keras.layers.Dense(
    progroup_vectorizer_layer.vocabulary_size(), 
    activation=sigmoid, 
    name=output
)(x)

model = tf.keras.Model(
    inputs=[input_new_skills, input_job_titles], 
    outputs=outputs
)
...
BUCKET = 'gsmentor-pilot-project-bucket'
model.save(BUCKET + 'mpgmodel_dave')
